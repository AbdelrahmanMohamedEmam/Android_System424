...
void main() async{
  ...
  group('Track API Test',(){
    test('Recently played false', () async{
      ...
      expect(trackAPI.addToRecentlyPlayed('contextUri', 'trackUri', 'album', 'token'),throwsA(isInstanceOf<Exception>()));
    });

    test('Recently played', () async{
      final trackAPI= new TrackAPI(baseUrl: 'http://spotify.mocklab.io');
      final item=await trackAPI.addToRecentlyPlayed('contextUri', 'trackUri', 'album', 'token');
      expect(item,true);
    });
});
}
